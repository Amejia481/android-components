...
import mozilla.components.feature.scroll.ScrollFeature.Companion.DEFAULT_SCROLL_FLAGS
...
class ScrollFeatureTest {
    private lateinit var scrollFeature: ScrollFeature
    ...
    fun setup() {
        ...
        scrollFeature = ScrollFeature(mockSessionManager, mockEngineView, mockView)
    ...
    fun `when session loading StateChanged and engine canScrollVertically is true must add custom scrollFlags`() {
        ...
        val session = getSelectedSession()
        `when`(mockEngineView.canScrollVerticallyDown()).thenReturn(true)
        scrollFeature = ScrollFeature(mockSessionManager, mockEngineView, mockView, 12)
        scrollFeature.start()
